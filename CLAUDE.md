...
## 17) Do not do the following without user approval: 
If a process is failing due to a port being used, prompt the user. Do not start on a different port. 
Do not create simpler implementations if the one that was created is failing.
For testing, prompt the user to test for you, do not run tests on your own without explicit approval. Only the user can provide instructions to run test. 