...
const outputFile = path.join(testOutputDir, `all-tests-${dateStr}-${timeStr}.txt`);

console.log(`Running all workspace tests and saving output to: ${outputFile}`);

// Start the test process
const testProcess = spawn('npm', ['run', 'test', '--workspaces'], {
...
outputStream.write(`All Workspace Test Results - ${now.toISOString()}\n`);
...
testProcess.on('close', (code) => {
  outputStream.write(`\n\nAll tests completed with exit code: ${code}\n`);
